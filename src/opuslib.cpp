...
godot::PackedByteArray OpusLib::convert_signed_pcm(const godot::PackedVector2Array &pcm) {
	...
	arr.resize(pcm.size() * sizeof(int16_t)); // 2 bytes per sample (2 channels * 2 bytes per sample)
	...
	for (size_t i = 0; i < pcm.size(); i++) {
		...
		data += sizeof(int16_t); // Move to the next sample (2 channels)
	}
...
godot::PackedVector2Array OpusLib::convert_unsigned_pcm(const godot::PackedByteArray &pcm) {
	...
	size_t num_samples = pcm.size() / (sizeof(int16_t)); // 2 bytes per sample (2 channels * 2 bytes per sample)
